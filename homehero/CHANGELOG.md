...
### Fixed
- **Legacy database compatibility** (`src/models/Task.js`)
  - Fixed NOT NULL constraint failure on 'type' column for databases created before v1.2.0
  - Fixed CHECK constraint failure - old schema requires type IN ('daily', 'weekly', 'one-time')
  - Task creation now detects legacy schema and provides valid default value for 'type' column
